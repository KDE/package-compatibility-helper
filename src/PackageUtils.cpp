...
bool extractMetainfoFiles(const QUrl &archivePath, QStringList &metainfoFilesContent)
{
    ...
    const QStringList listArgs = QStringList() << u"-tf"_s << archivePath.toLocalFile();

    listProcess.start(u"bsdtar"_s, listArgs);
    listProcess.waitForFinished(-1);

    if (listProcess.exitStatus() != QProcess::NormalExit || listProcess.exitCode() != 0) {
        qWarning() << "Error listing archive contents:" << listProcess.readAllStandardError();
        return false;
    }

    const QStringList allFiles = QString::fromLocal8Bit(listProcess.readAllStandardOutput()).split(u'\n', Qt::SkipEmptyParts);

    QStringList metainfoPaths;

    for (const QString &file : allFiles) {
        if ((file.startsWith(u"./usr/share/metainfo/"_s) || file.startsWith(u"./usr/local/share/metainfo/"_s)) && file.endsWith(u".xml"_s)) {
            metainfoPaths.append(file);
        }
    }

    if (metainfoPaths.isEmpty()) {
        return true; // Valid archive, just not an app
    }

    for (const QString &file : metainfoPaths) {
        QProcess extractProcess;

        const QStringList extractArgs = QStringList() << u"-xOf"_s << archivePath.toLocalFile() << file;

        extractProcess.start(u"bsdtar"_s, extractArgs);
